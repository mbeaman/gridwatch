...
> **Status: arc 1 is on `main` (CI green); arc 2 is built — 2a (journal, pid scan, htop table, tooling; D48) and 2b (the gpu source over NVML, the gpu component from badge to the zoom-only `full`, the braille chart renderer, the nvtop PARITY section; D49) on 2026-09-01. `v0.1.0`/`v0.2.0` are deliberately NOT tagged yet: P4/P21/P9/P10 need a human in Ptyxis (see `PERFORMANCE.md`'s owed list), as do the glyph check (braille now matters) and the README PNG. Arc 3a (the pins tile over astral-watch, the alert banner/ack/`alerts` tile, `synth-overload.jsonl`; D50/D51) and 3b (hot reload, theme loader v2 with `terminal`/`phosphor-green`, `STALE` badges, placeholder fix lines, `gridwatch doctor`; D52/D53) are built and gated 2026-09-02 — owed to a human: P14's live number and `doctor`'s live table (both open `/dev/i2c-*`), P1/P5/P6 with the *live* pins source (measured under `--demo` here), and `v0.3.0`. Next: arc 4 (edit mode, effects, `matrix`) needs a brief; `fixtures/journals/torch-game.jsonl` still needs Matt to start the game.** Repo: `github.com/mbeaman/gridwatch` (the on-disk directory remains `~/workspace/opsTui`; see `CLAUDE.md` for why). This file is the entry point — read it first in every session, then `ROADMAP.md` for the current arc and the tail of `DECISIONS.md`.
