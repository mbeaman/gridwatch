...
> **Status: arc 1 is on `main` (CI green); arc 2 is built — 2a (journal, pid scan, htop table, tooling; D48) and 2b (the gpu source over NVML, the gpu component from badge to the zoom-only `full`, the braille chart renderer, the nvtop PARITY section; D49) on 2026-09-01. `v0.1.0`/`v0.2.0` are deliberately NOT tagged yet: P4/P21/P9/P10 need a human in Ptyxis (see `PERFORMANCE.md`'s owed list), as do the glyph check (braille now matters) and the README PNG. Arc 3a (the pins tile over astral-watch, the alert banner/ack/`alerts` tile, `synth-overload.jsonl`; D50/D51) is built and gated 2026-09-02 — P14's live number is owed to a human (the pass opens `/dev/i2c-*`). Next: session 3b (hot reload, theme loader v2, staleness, `doctor`) from `docs/briefs/arc-3.md`; `fixtures/journals/torch-game.jsonl` still needs Matt to start the game.** Repo: `github.com/mbeaman/gridwatch` (the on-disk directory remains `~/workspace/opsTui`; see `CLAUDE.md` for why). This file is the entry point — read it first in every session, then `ROADMAP.md` for the current arc and the tail of `DECISIONS.md`.
