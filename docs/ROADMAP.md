...
- [~] Fixtures: `torch-idle.jsonl` recorded (2a, cpu only, 60 s); **`torch-game.jsonl` owed — no game was running during 2a, and the brief forbids starting heavy jobs**; determinism test replaying through `run<TestBackend>` twice with identical frame hashes (2a); `docs/PARITY.md` nvtop section (2b)
