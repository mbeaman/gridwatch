...
- **A plugin's read budget as config** (arc 8b): `MAX_MSGS_PER_SEC` is a const (500), while `render_ms`, `hello_ms`, `rss_mb` and `cpu_secs` are per-plugin config. 500 is far above what any plugin here needs, but one legitimately publishing at 60 Hz across a dozen keys would reach it and be throttled without being told. If a real plugin ever does, this is the knob to expose — and the tile should say when the budget is what is slowing it, which today is only a `debug` log line.
