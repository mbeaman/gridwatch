...
- **`torch-game.jsonl` fixture** (arc 2b or whenever Matt has the game up): `gridwatch run --record fixtures/journals/torch-game.jsonl` for 60 s at 250×70 with tables off; 2a could only record the idle box (no heavy jobs from an agent, MACHINE.md).
