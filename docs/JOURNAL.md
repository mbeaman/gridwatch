...
### The documentation debt this session created

Worth separating from what it found, because it is ours. **The published wiki is public
and stale**: it went up on 09-12, and arcs 16 and 17 have both changed it since — every
per-tile screenshot regenerated, and `Tiles-Disks.md` now describes a tile that does not
behave as written. `scripts/wiki-publish.sh` fixes it in one command and is outward-facing,
so it waits. The standing hazard is bigger than this instance: **anything touching
`wiki/` or `docs/img/wiki/` leaves the live copy wrong until someone remembers**, and
nothing warns.

Alongside it: four wiki pages are still the honest stubs the first publish shipped, and
arc 17's behaviour — a device that goes quiet drawing dashes and sinking — appears in no
user-facing sentence at all. New visible behaviour, documented only in a decision file.

### Two working-practice defects, both hit repeatedly today

**`scripts/gate.sh`'s drift step cannot pass before a commit.** It regenerates and then
asks `git diff`, so uncommitted regeneration is indistinguishable from real drift. Any arc
that touches a generated artifact therefore fails its own gate until the work is
committed, which inverts the project's own "gate before any commit" rule. This session hit
it **six times** and each time the way through was to commit and re-gate — i.e. to do the
thing the rule exists to prevent. The fix is for `--check` to diff the regenerated bytes
against what it replaced rather than against `HEAD`.

**And nothing enforces "never a bulk accept".** `REVIEW.md`'s checklist asks for snapshots
accepted one by one; `cargo insta accept` takes them all and no gate notices. Arc 16 and
arc 17 both inspected by category and then bulk-accepted — the same inspection, the wrong
mechanism, twice, by the session that wrote the rule down.
...
- **Republishing the wiki**, one command, outward-facing.
