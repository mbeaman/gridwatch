...
**P17 after arc 16 (2026-09-14) — owed, with the arithmetic that says why it is probably fine.** Arc 16's brief and ROADMAP both named "P17 re-taken under `--demo`" as a gate, because a richer `demo::NetSynth` and `demo::DiskSynth` put more series in the demo store. **The gate was not taken and the arc did not record it as owed** — its own review found that. Taking it honestly needs a 1 h run under a pty, which is the P17 protocol; what can be stated without one is the arithmetic. The fixtures added **three interfaces × 4 scalars = 12** and **two devices × up to 9 scalars = 18**, so ≈ **30 scalar series** on a base of roughly 150, plus one larger `net.conns` Record (32 rows where it held 5). At the shipped `history = "10m"` a scalar series preallocates 38 KB, so the added floor is ≈ **1.1 MB** against a 60 MB budget with 40.3 MB measured at arc 14. That is a forecast, not a row: **the measured row is owed, and it is `--demo`-only — no live path gained a series.**
...
**Arc 17 and 17b (2026-09-16 / 2026-09-18) — asserted unchanged, not re-measured, and marked as an assertion.** Arc 17 wrote no performance line at all (its own review found that, lens D), so this is the record. **P2, P4, P5, P13, P15, P17, P18 and P19 are unchanged and asserted so.** The whole-arc diff touches no `Store::apply`, no renderer, no `app` and no source (`git diff 027be22..HEAD --stat -- crates/store/src/store.rs crates/ui/src/renderer.rs crates/app crates/sources` is empty; the only `gridwatch-store` change is a `pub const` and a comment in `demo/disk.rs`, which no live path reads). What each tile now costs, from reading rather than a timer: one extra `Pulse::observe` per `tick` (one `last_sample` lookup — so each tile does that lookup **twice**, three times in `sensors`, where D68 §6 said one; nanoseconds, and recorded as a `BACKLOG.md` smell rather than a cost), and per *rebuild* — which only runs when the source publishes, never per frame — a subtraction and a comparison per drawn label (`sensors` judges each reading against one pulse, or two for `k10temp:*`). The `disk` synth's cycle went from 60 s to 180 s (D68 §7), which changes what a `--demo` run *shows* at 45–180 s and nothing it costs. **Still owed, and it is arc 16's debt rather than this arc's:** the one-hour `--demo` P17 re-take (`BACKLOG.md`); arc 17 added no demo series, only lengthened the removable's cycle.
