...
Sources are **singletons per kind**, configured only under `[sources.<id>]` in `config.toml`; a `Command::Source(audio, Domain(SetSink))` therefore changes the sink for every audio tile. `Level::Paused` (htop's `Z`) makes sources emit nothing except `always_on` ones (pins). `detail` is the max of `Component::demand(tier)` over the visible instances that list the source (own or optional): `Meters` costs nothing extra, `Table` turns on the pid-level process scan, `Columns` is reached only by a `full` tier (zoomed or `view = "full"`) and is refined per column — the component sends `Command::Source(cpu, Control::SetOption("threads", bool))` when `H` is pressed, so the cpu source walks `task/` only when userland threads are shown — htop's `PROCESS_FLAG_*` idea, and the only such control that exists. *(As built in 10b: the key is `"threads"`, not the `"columns"` this line said before it was implemented; the gated files — `io`, and `smaps_rollup` if it is ever read — are still gated by `Detail::Columns` alone rather than per column, because the walk is the one that costs enough to need a second gate. The walk needs **both**: `Detail::Columns` and the control, because htop's I/O screen raises the same detail for `/proc/<pid>/io` and has no use for threads.)* `FocusLost` drops every source to `Hidden` and `Meters` (pins stays `always_on`). Replay is one `JournalSource`; demo wraps `store::demo::Synth` per source.
...
**As built (arc 7b, `crates/store/src/rules.rs`).** The engine is `Rules { rules: Vec<Rule>, states: BTreeMap<(usize, String), State>, started: Option<Ts> }` — one state per rule *and label*, so `sensor.temp_c{nvme*:Composite}` is one rule and three alerts (`<name>/<label>`). `Store::apply` calls `Rules::observe` over the scalars a batch carried and returns the events; `Store::tick_rules(at)` handles the `absent` rules on the frame's clock and is skipped entirely unless `Rules::has_absent()`, with the label lookup a **range** over `MetricId`'s name-first ordering rather than a walk (3.7 µs a frame over 2 000 series). A rule's right-hand side may be a number or another metric read at the same label; a right-hand side that has not arrived raises nothing. Every rule alert carries `source = SourceId("rules")`, a pseudo-source no registry knows — the banner, the alerts tile and `A` render it like any other. `Shell::route_alerts` is the one path from an `AlertEvent` to a toast and the acked set, used by both `apply_control` and the frame loop's batch drain. Equality compares with a relative tolerance, not `f64::EPSILON`.
...
**As built (arc 7a, the network vertical).** `sources::net` is four independent parsers over a `Roots { proc, sys, resolv }` so every one of them is a pure function of a directory tree and testable from `fixtures/net/`: `dev` (rates from the measured interval, `saturating_sub` for a re-created interface), `link` (sysfs attributes; a speed is unknown without a carrier), `route` (little-endian hex, systemd-resolved's stub named for what it is) and `conns` (`/proc/net/{tcp,tcp6,udp,udp6}` joined with an inode map from `/proc/*/fd`, at `Detail::Table` only and on its own cadence, reporting how many of how many sockets it could attribute). `probe` opens an **unprivileged** `SOCK_DGRAM` ICMP socket through `socket2` and converts it to a `std::net::UdpSocket` so no `MaybeUninit` (and no `unsafe`) is needed, falling back to a TCP connect and saying so. The glob rule every filter uses — chips, interfaces, rule labels — is `store::rules::glob`, one implementation with a `*` anywhere (D57 amendment 9).
...
**Drawings scale with their rect (D62).** A drawing inside a `Fill` band takes its size from the rect it is given — a bar's width from the block, a chart's rows from the band, a sparkline's columns from the width — and a constant is only ever a *floor*, never a cap: a tile at 480×135 must be as full as one at 250×70. A `View::Sparkline` column with no sample (`None`) draws the newest sample before it, the way htop's and nvtop's one-sample-per-column meters do; only columns before the first sample are empty. The testkit's `assert_grows_with_area` enforces the rule per listed tier (double the width ⇒ ≥ 1.5× the drawn cells), and `assert_renders_everywhere` still sweeps the sizes.
...
**What bounds a plugin (D39, D58 seam 7).** No shell, ever; every line validated before it is read; a 1 MiB line cap; `RLIMIT_AS`/`RLIMIT_CPU` and `env_clear` on the child; three malformed messages stop it rather than restart it. Two of the bounds are what P22 turns on: the inbound queue is **64 deep and drops the oldest droppable message** — a sample, a view or a log line, never a manifest or a status (a reading nobody has read yet is worth less than the one after it; a declaration is not), and the reader takes at most **`MAX_MSGS_PER_SEC` (500)** messages a second from one plugin — stopping *before* the read, so its pipe fills and the child blocks in `write` rather than either process spinning. For a plugin that burns a core without writing, the host reads each child's `utime + stime` once a second and stops one holding **50 % of a core for 10 s**. Metric names are interned and capped at **256 per plugin**, because a `MetricId` name is `&'static str` and an unbounded set of them is an unbounded leak; labels are not leaked and are not capped.
