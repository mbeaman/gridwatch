...
Domain alerts arrive as `ControlMsg::Alert` (the pins source runs astral-watch's `Lifecycle`, tracks the active `Condition` set itself and emits transitions); `[[rules]]` from `config.toml` run inside `apply` on touched metrics via a name-indexed map (NaN/absent never raise or clear). The overlay keys on `AlertId` transitions, never on samples.

### 4.5 Journal

JSON Lines: a header (`version`, `wall_epoch`, `host`, `sources`, terminal size) then one line per `Batch`, `Status`, `Alert` (input only with `--record-input`). Scalars/vectors are plain JSON; records use `RecordValue::to_json` on write and `KeyMeta::decode` on read; names are interned through `lookup`. `Replay` drives the virtual clock; `apply_all`/`apply_until` feed tests without timing. `--tables off` omits only `proc.table` and keeps fixtures ~1 MB per 10 minutes. A round-trip unit test iterates every `KeyMeta` with `kind == Record` and asserts `decode(to_json(x)) == x`.
...
| gpu (NVML) | fast tier 1 s / 500 ms / 250 ms (≈ 20 µs); slow tier 1 s (memory, enc/dec, PCIe counters); fans %/RPM every 5 s; `samples(Power)` only at `header`+ tiers; process rows only at `Detail::Table`; static once | never `pcie_throughput` (21 ms); byte-counter fields 197/198; keys labelled `{0}`; nvtop refreshes at 1 s — slow tier ≈ 2.3 ms/s, ≈ 4.3 ms/s with process rows |
| pins (astral-watch) | 1 s / 500 ms / 500 ms, `always_on` | never paused; alerts depend on it |
...
pub fn hit(s: &Solved, pos: Position) -> Option<&Cell>;  pub fn unit_at(…) -> Option<(u8, u8)>;  pub fn focus_dir(…) -> Option<InstanceId>;
...
impl Theme {
    ...
    pub fn for_component(&self, kind: &str) -> Arc<Theme>;
...
| `USER` | off (`columns`) | 10 + 1 | `user` | left-aligned, muted when not our uid, `AccentTertiary` when the process has elevated capabilities |
...
| `S` | **on** | 1 + 1 | `state` | `R`/`U`/`t` → `Ok`; `D`/`Z`/`T`/`X`/`B` → `Crit`; `S`/`I`/`W`/`Q` → `TextMuted`; `?`/`P` → `Text` (htop prints `P` as `B`) |
| `CPU%` | **on** | auto: 4 + 1, growing to `ceil(log10(max_seen + 0.1)) + 2` and never shrinking within a run (6 on torch under a game) | `cpu_pct` | `< 0.05` muted, `≥ 99.9` accent; Irix mode, up to `active_cpus × 100` |
...
|---|---|---|---|---|
| `PID` | **on** | 7 + 1 | `pid` | |
| `USER` | off (`columns`) | max(4, longest name among the visible rows) + 1 | join `proc.table` | nvtop's rule: minimum 4, never truncated (8 on torch) |
...
Hot reload: a watcher thread `stat()`s `config.toml`, `layout.toml` and the active theme once per second (no `notify`, immune to editor renames); on change it parses and validates on the watcher thread and sends `ControlMsg::Reload`; the shell keeps instances whose `(kind, options)` are unchanged, rebuilds the rest, swaps `Arc<Theme>`, and on error keeps the old config with a toast. Edit-mode saves go through `toml_edit::DocumentMut` (comments preserved), atomic temp+rename, re-parse verification, and a content hash the watcher ignores.

## 10. UX and keys

Global: `Ctrl-q` quit, `?` help, `F12` stats HUD (frame time p50/p95, changed cells, bytes written, mode). Grid mode: `1–9` pages, `[`/`]` prev/next, `Tab`/`Shift-Tab` reading-order focus, `h j k l`/arrows spatial focus, `Enter` capture keys into the focused component (its `keys` replace the status bar), `Esc`/`Outcome::Release` returns, `z` zoom, `d` dense (overrides the size-derived mode for the session), `t` cycle theme, `T` reload theme, `V` re-light the page (showcase themes), `L` lock everything lit / unlock, `space` pause (Level::Paused; pins keeps sampling), `r` record toggle, `a` ack banner, `A` alerts tile, `S` screenshot to `~/.local/state/gridwatch/shot-<ts>.txt`, `e` edit mode, `q` quit when not captured. Mouse (SGR, opt-out; Shift-drag keeps native VTE selection): click focus, double-click zoom, wheel forwarded with local coordinates. Edit mode: `H J K L` move, `Ctrl-hjkl` resize (Shift shrinks), `s` cycle `manifest.footprints`, `S`+dir swap, `a` picker (existing instances or a `kind:` shorthand → `insert_first_fit`), `x` remove placement, `u`/`Ctrl-r` undo/redo (page snapshots), `w` save, `Esc` leave (prompt if dirty); collisions rejected with a red ghost; drag moves, corner drag resizes. Only the legacy keyboard encoding is assumed (VTE has no kitty protocol).

## 11. Error handling and degraded modes

- **Capability probe** (`CapSet`, ≤200 ms, per-check timeout, cheap checks only) + `Manifest.requires/optional`: a missing required capability skips `build` and installs a placeholder whose tiers show reason and fix (`usermod -aG i2c`, udev rule, `apt install pipewire-bin`). `gridwatch doctor` prints the table.
- **Runtime loss** → `SourceStatus { state: Starting|Ok|Degraded|Unavailable|Stopped, reason, hint, since, last_sample, dropped, restarts }`; tiles whose `last_sample` age exceeds 3× cadence are dimmed with a `STALE 12s` badge; the `sources` tile shows everything.
...
- **stderr** is `dup2`'d to `$XDG_STATE_HOME/gridwatch/gridwatch.log` before entering the alternate screen (astral-watch's library `eprintln!`s would otherwise scribble on the UI); `tracing` goes to the same file.
...
5. **Demo mode**: `gridwatch --demo [--seed N]` swaps every source for its seeded `Synth` (32 cores with a game-like load, a 5090 at 400 W, a 1.5× idle pin imbalance with a scripted overload at t=40 s, a synthetic stereo mix, a fake Firefox track with embedded art). `gridwatch shot --demo --theme retrowave --size 250x70 --page 1 --at 45s --format ansi|svg --out …` renders headless; once the SVG dumper exists CI regenerates README images and fails on drift.
...
4. Tests: `snapshot_matrix!`, `assert_never_panics`, `assert_min_tier_fits`, a replay assertion if a fixture exists; `cargo insta review`.
