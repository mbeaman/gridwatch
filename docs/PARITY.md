...
| `hide_kernel_threads` (default on), `PRI` prints `RT`, `NI` colours, thread rows in the thread role | **in — arc 2a, exercised from arc 10b** | `htop::table::cell`; thread rows exist from `Detail::Columns` with `H` on (arc 10b) — the filter and the role had been in place and unexercised since arc 2a |
...
| Search `/`, filter `\\`, tags, tree (`t` or `F5`), follow, `H`/`K` toggles | **in — arc 8a** (`full`, zoom-only) | `Htop::full_key`; `crates/components/tests/htop_full.rs` pins each one, including that none of them answer on the un-zoomed grid tile. Deviations, all deliberate: search jumps the cursor and does **not** filter (htop's `/` does the same); `Esc` clears a filter rather than leaving a hidden one in force; the tree is a re-ordering computed in `tick`, so a filtered tree keeps the rows whose parents are gone as roots instead of hiding them. `H` lists each thread as its own row from arc 10b — the `task/` walk (`procs::walk_tasks`), gated on `Detail::Columns` **and** the toggle, because the I/O screen raises the same detail and wants no walk. One **deviation**: the Command cell of a thread row carries the thread's own `comm`, where htop shows the leader's cmdline unless `Show custom thread names` is on — the comm is the only per-thread identity there is, and a screen of identical cmdlines is what the feature exists to avoid. A thread row's `ppid` is its **leader**, so `t`/`F5` nests it under the process as htop does |
| Horizontal scroll of the process table (`h`/`l`) | **out for htop, in for the gpu tile** | htop's Main screen fits its columns to the width and drops them in a documented order (§8.1), so there is nothing to scroll past; nvtop's does not, and the gpu `full` tier scrolls four at a time with `PID` and `Command` pinned |
...
| Full Main screen, I/O screen, tree view, search, filter, tags, follow, `K`/`H` | **in — arc 8a** (`full`, zoom-only) | Two screens as tabs (`Tab` switches). htop's user-configurable screen *set* is out: it is a config surface nobody has asked for. The I/O screen reads `/proc/<pid>/io`'s **block-layer** counters (`read_bytes`/`write_bytes`, not `rchar`/`wchar`) at `Detail::Columns` only, and marks the rows it could not read `n/a` rather than showing zeroes |
| htop's F-key bar | **in — arc 8a** as spans through theme roles (`F5` tree · `F6` sort · `F7`/`F8` nice · `F9` kill), with the letter keys named beside it. `F1`–`F4` and `F10` are htop's help/setup/search/filter/quit and are **not** bound: this tile lives inside a dashboard whose own keys own that space | `htop::full::fkey_bar` |
| `k` as a direct SIGTERM | **out — deliberate**: `k` is the vi cursor key in both process tables, and htop's own `F9` menu is one keystroke away. Recorded here rather than left as silent drift (arc 8a review) | |
| Kill / renice / affinity / ioprio | **in — arc 8a**, behind `readonly` and a confirm line | The component builds a `ProcAction` (data); the shell asks the question; the executor thread makes the syscall (`app::proc_actions`). `EPERM` reads "not yours to change", `ESRCH` "it exited first". Pid 0, pid 1 and gridwatch itself are refused outright. An action applies to the tag set when there is one, else to the cursor's row — htop's rule. `F7`/`F8` are the one action that does not ask, because the opposite key undoes it |
...
| Charts: 10-minute ring buffer, one column per sample (so the window is `cols × interval`), fixed 0–100 % axis with quarter ticks | **in — arc 2b, completed in arc 15**. Arc 2b's row claimed the axis and drew only its *range*: the ticks were rendered by nothing and the row did not say so. From arc 15 the renderer draws the **quarter rules** (0.25/0.5/0.75) from `Bounds.y` for every `View::Chart` caller, unlabelled — nvtop's 0 % and 100 % ticks are the band's own edges here and are not drawn as rules — `Bounds` carries no unit and giving it one is a §4.6 change (BACKLOG) with a deliberate deviation: gridwatch always shows the last ten minutes resampled into one bucket per cell, whatever the width (`charts` tier; the band takes every row the header leaves — the table's rows too at the table tiers — floored at 4 and never capped — D62) | `store.resample` over `CHART_SPAN`, `View::Chart` drawn as connected braille segments by the theme's `chart_marker`; the window label follows the run's age up to `10m` |
...
| `F9` "Send signal" menu | **in — arc 8a** with the nine signals htop lists first, behind the confirm line. The full 1–31 list is out: the rest are a `kill -N` away and a dashboard menu of `SIGPWR` is not what anyone needs | `gpu::view::signal_menu`, `htop::full::menu_view` |
...
| `sensors` names chips `k10temp-pci-00c3` (driver + bus) | **deviation**: chips are keyed by hwmon `name` with a `#2`/`#3` suffix for duplicates, **ordered by the device they hang off** (`hwmonN/device`: `nvme0`, `0000:00:18.3`, `8-0051`) so a drive keeps its name across reboots; the device rides in `sensor.info` | `hwmon::walk`, `hwmon::device_of`; D55 5b amendments 28 |
...
| `sensors` shows every reading always | **deviation**: the tile ranks by **closeness to a limit** — the chip's `max`, or an assumed ceiling (95 °C for k10temp's Tctl/Tccd, 100 °C otherwise, marked `~`) — with anything past `crit` then past `max` first, and the table prints that same percentage so the order is visible; `o` switches to a by-chip order | `components::sensors::hottest_first`; D55 5b amendment 29 |
...
| `iostat -x`: `%util` | **deliberate rename**: `BUSY`, per device, clamped 0–100, with the caveat printed in the zoom-only `full` pane (not on the grid tiers — §8 is the accurate scope) — on torch a bounded `dd` at 524 MB/s read **7.3 % util** with a 0.38 average queue, while one I/O outstanding on a 1023-deep queue reads 100 % (measured over a live interval — field 14's lifetime value is 32-bit truncated and has wrapped here; D64 amendment 1) | D64 §6 |
