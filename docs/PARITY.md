...
| Search `/`, filter `\\`, tags, tree, follow, `H`/`K` toggles | **in — arc 8a** (`full`, zoom-only) | `Htop::full_key`; `crates/components/tests/htop_full.rs` pins each one. Deviations, all deliberate: search jumps the cursor and does **not** filter (htop's `/` does the same); `Esc` clears a filter rather than leaving a hidden one in force; the tree is a re-ordering computed in `tick`, so a filtered tree keeps the rows whose parents are gone as roots instead of hiding them |
