...
Arc 5a's audio vertical: `crates/sources/src/audio/dsp.rs` carries the §12.1 tests (a full-scale 1 kHz sine lights its band ≥ 0.97 and reads −3.01 dBFS RMS, a bin-centred 52.7 Hz sine leaves exactly one dominant band below 100 Hz, silence is all zeros and a short history pads, the Hann `2/Σw` identity reads 1.0 for a bin-centred sine, the tilt and the config clamps, the peak hold's 1.5 s and 20 dB/s); `capture.rs` pins the verified `pw-record` command line (with `stdbuf -o0 … --latency 256` and `node.dont-fallback` under their options) and pumps a generated f32 stream through the ring across odd-sized reads and a full ring; `sink.rs` parses a `pw-dump` excerpt shaped like torch's (serials, never node ids; the `default` metadata's props at the top level); `supervise.rs` drives the `Policy` (spawn on the first visible demand, kill 10 s after hidden, respawn on EOF with the 250 ms → 5 s backoff, never on "no data") and the `Silence` rule (250 ms without a frame, 500 ms below the floor, back at once) on fixed instants; `mod.rs` runs the publisher over a ring of a 1 kHz tone and then silence, and the `#[ignore]` `live_pw_record_delivers_frames` captures two seconds on torch (a safe read-only probe). `crates/components/src/audio/ballistics.rs` tests the Winamp falloff and peak schedule and cava's integral/monstercat/gravity at 30 and 60 fps; `crates/components/tests/audio.rs` pins the tier per real grid size, the keys, the sink picker's `Domain` command (the boxed `SetSink` downcasts), `Animated` with sound and `Redraw::No` once silent and settled, the empty and `Unavailable` tiles, and the mirrored bar layout; the component snapshot matrix gained the audio tile at every grid size. The shell tests' staleness rows count the audio tile (its cadence follows `fps` and the silence rule). `crates/app/tests/replay.rs` replays `torch-audio.jsonl` (the silence path) deterministically. Arc 7a's network vertical: the parsers are pure functions over a root path and every awkward thing the digest found on torch is a test — `dev.rs` splits `/proc/net/dev` on the **first colon** (the fixture holds the `br-6bb7413a559e:      84` line that whitespace-splitting would mangle), turns counters into rates over the measured interval and treats a counter that went backwards as a re-created interface rather than a negative rate; `link.rs` reads the sysfs attributes and refuses to invent a speed (the down radio's `speed` is unreadable, and a carrier-less link's number was negotiated with nobody), recognises a radio by its `wireless` directory and a bridge by what hangs off its own; `route.rs` decodes the little-endian gateway (`0164A8C0` = 192.168.100.1), prefers the lowest metric and labels systemd-resolved's stub for what it is; `conns.rs` decodes both address families (four little-endian words for IPv6, big-endian ports, the `st` state table) and attributes sockets through `/proc/*/fd` — its test opens a listener and finds *itself* in the table, which is the one process any machine can read; `probe.rs` pins the ring statistics (min/avg/max/mdev, RFC 3550 jitter, loss, a bounded ring) and measures a TCP probe against a listener it opened, with the ICMP echo left to the `#[ignore]` `live_icmp_reaches_the_gateway`. `crates/components/tests/net.rs` pins the tier per real grid size, the hide list and the `a` key, the sort, the probe strip, the connection table's honest attribution (a process where `/proc/<pid>/fd` was readable, a uid where it was not), the `full` tier's route and probe statistics, the empty tile, and that a quiet interface reads `0B` rather than a dash. Pty **C.23** finds the tile and its probe strip under `--demo` and checks `doctor --offline` counts the real interfaces; **C.24** reloads a single-tile layout, captures it, and presses `a` (the hidden bridge appears) and `s`.

Arc 6's media vertical: `crates/sources/src/mpris/meta.rs` decodes `a{sv}` from `fixtures/mpris/` — Firefox's one-element `xesam:artist` array, its empty album, an object-path `mpris:trackid`, `mpris:length` in µs, a stream with no length at all, a missing title falling back to the URL's last part, two artists joined, and length as a double or a string; `model.rs` is the player state machine — the brief's fake `Bus` trait was not needed, because every lifecycle rule is a pure function of the events the loop feeds in — and it carries those rules as tests (Playing beats the most recent beats the name, a pin overrides both and lapses when its player leaves, only the current *playing* player is polled, a track change restarts the clock and writes the capped history, a stream runs a local elapsed clock and position noise is not republished, art is wanted once per track, a nonsense rate or volume is clamped); `art.rs` decodes and downscales a generated PNG, reads `file://` with percent-escapes, `data:` in base64 and plain, and refuses `ftp://` and a missing file; `mod.rs` pins the option clamps and the zbus-value → decoder-shape conversion, and carries the `#[ignore]` `live_mpris_lists_players` probe. `crates/ui/src/halfblock.rs` tests the two-pixels-per-cell painter (fg is the top pixel, bg the bottom), the scale up and down, the 256- and 16-colour quantisation and mono's luminance shades. `crates/components/src/winamp/marquee.rs` pins the 220 ms step, the `  ***  ` separator, "no scroll when it fits", never splitting a wide glyph, and the clock/remaining formats; `crates/components/tests/winamp.rs` pins the tier per real grid size, every transport key as a `Domain` command (and a player that can do nothing consuming them), the animation rule, stream mode, the idle skin with the source's reason, the vis with and without the audio source, and the art column appearing only for a matching track. Pty **C.21** runs `--demo --page 2`, waits for a track on the tile and checks the stats log counts animation frames; **C.22** captures the tile, checks the transport hints reach the key bar and presses them.

Arc 5b's sensors vertical: `crates/sources/src/sensors/hwmon.rs` walks `fixtures/hwmon/torch/` (copied from torch's `/sys/class/hwmon`) and pins the inventory — nine chips with `nvme#2`/`nvme#3`/`spd5118#2` numbered by name, the attribute-less `asus` skipped, `nvme:Sensor 1`'s `65261850` m°C `max` dropped as a sentinel, an unlabelled input falling back to its stem, and the unit divisions (m°C, mV, µW); `rapl.rs` drives the three states and the wrapping `Δenergy` on a temp tree; `sensors/mod.rs` samples the fixture (fifteen readings, the thresholds and the inventory once, nothing republished on an unchanged re-walk) and parses the options. `crates/components/tests/sensors.rs` pins the tier per real grid size, the hottest rule (the smallest margin to `max`, so an 80 °C NVMe outranks a 59 °C Tctl with no max), the over-max/over-crit marks, the `o` sort and the chip filter, the `full` tier's RAPL/PSI/gpu rows with and without the gpu source, and the honest empty tile; the component snapshot matrix gained the sensors tile. Pty **C.20** runs `--demo` and waits for a chip name on the Overview, then runs `doctor --offline` and checks the hwmon row (the walk is a sysfs read, so it runs even offline).
