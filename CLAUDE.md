...
Modular, themeable ops-dashboard TUI for Matt's workstation "torch", in Rust (ratatui 0.30). **Status: arc 1 is committed and pushed; arc 2 — 2a (journal, pid scan, htop table, tooling — D48) and 2b (the GPU — D49) — is built, reviewed and gated (2026-09-01). `v0.1.0`/`v0.2.0` are untagged pending the rows only Matt can close (P4/P21 in Ptyxis, the glyph check incl. braille, the README PNG). Arc 3 — 3a (pins + alerts, D50/D51) and 3b (hot reload, theme loader v2, staleness, doctor; D52/D53) — built and gated 2026-09-02; P14's live pass and `doctor`'s live table are Matt's (they open `/dev/i2c-*`), as is `v0.3.0`. Arc 4 (edit mode, effects, `phosphor-amber`, `matrix`; D54) built and gated 2026-09-02 autonomously (Matt: proceed without a human in the loop; tags and the beside-the-game/Ptyxis/i2c rows stay his). Arc 5a (the audio visualizer over `pw-record`; D55) built and gated 2026-09-02 autonomously — the "with sound playing" P16 row and the Firefox/game reaction row are Matt's (an agent starts no player). Arc 5b (sensors + the k10temp handover) built and gated 2026-09-02. Arc 6 (mpris + winamp; D56) built and gated 2026-09-02. Arc 7 — 7a (the network vertical) and 7b (the `[[rules]]` alert engine); D57 — and **arc 8a** (the action executor, the four process actions, htop's `full` tier, the gpu tier's scroll and signal menu; D58) are built, reviewed, fixed and pushed 2026-09-02, CI green. **Arc 8b's plugin host is built and tested but not yet wired into the app** — the schemas, the supervisor, the view-tree reader, the example plugin and `docs/PLUGINS.md` are on `main`; `[[plugins]]` config, registry placement and rendering are what remain (see `arc-8-state` in Claude memory and `docs/briefs/arc-8.md` seams 7–8). Note the 8a seam change: `InputCx` carries `zoomed` and `tier`, and a component must never infer its tier from `inner`'s size. Owed to Matt and never faked: the live Wi-Fi row (`wlp7s0` is down on torch), the public-IP and reverse-DNS paths, `torch-game.jsonl`, P9/P10 in Ptyxis, P14 live pins, P16 with sound, P17's 24 h run, and every tag from `v0.1.0` to `v0.7.0`.** Name: **`gridwatch`** (crates `gridwatch-*`). The on-disk directory remains `~/workspace/opsTui`; don't rename it (Claude memory is keyed to the path — migrate `~/.claude/projects/-home-mattbeam-workspace-opsTui/memory/` first if it ever moves).
...
- **Arcs.** One arc ≈ one coherent phase ≈ one minor version (`v0.N.0`), with a `CHANGELOG.md` entry. **Bump `Cargo.toml`'s version as part of the arc** — semver, one minor per arc — so the binary, the README and the docs never disagree (Matt, 2026-09-15). Tagging stays Matt's; the number does not wait for it. Arc 1 is planned as two sessions (1a/1b).
