...
**Status:** arcs 1–4 (`v0.1.0`–`v0.4.0`, untagged): the core seam, the 12×6 layout engine, the cpu source with htop's meters, CCD core bars and the top-N process table, **the gpu source over NVML with nvtop's header, ten-minute charts and GPU process table**, journal record/replay, CI-regenerated screenshots, and arcs 3–4: **the astral-watch pins tile with the cross-page alert banner**, hot reload of the config files and themes, the theme loader v2 with `terminal`, `phosphor-green` and `phosphor-amber`, `STALE` badges and `gridwatch doctor`, in-app layout editing, tachyonfx effects, and **`matrix`** — the showcase theme whose rain prints the dashboard (`--theme matrix`); and arc 5a: **the audio visualizer** — the default sink's monitor through a supervised `pw-record` child, a cava-style dual-FFT spectrum with Winamp or cava ballistics, an oscilloscope, stereo VU and a sink picker, animating at 30 fps (60 opt-in) only while there is sound. and arc 5b: **the sensors tile** — every hwmon chip's temperatures hottest-first, with the chip's own thresholds, RAPL, PSI and the GPU's row. and arc 6: **the now-playing tile** — the MPRIS player in classic-skin form, with a scrolling marquee, big elapsed digits, the transport row, the album art as halfblocks and the spectrum borrowed from the audio source. and arc 7: **the network tile** — every interface's rates and link state with rx/tx sparklines, the default route and resolvers, latency probes over an unprivileged ICMP socket, and the connection table with the process behind each socket where the kernel lets a normal user read it; plus **`[[rules]]`**, which turns any metric into an alert with holds, label globs and a threshold that can be another metric. What comes next is in [`docs/ROADMAP.md`](docs/ROADMAP.md).
