...
fn exemplar(name: &str) -> Option<Arc<dyn RecordValue>> {
    Some(match name {
        ...
        "net.link" => Arc::new(gridwatch_store::keys::net::Link {
            ...
            kind: gridwatch_store::keys::net::LinkKind::Ether,
            speed_mbps: 1000,
            carrier_changes: 2,
            addrs: vec!["192.168.100.154/24".into()],
            wifi: None,
        }),
        "net.route" => Arc::new(gridwatch_store::keys::net::Route {
        ...
        "net.conns" => Arc::new(gridwatch_store::demo::net_conns_exemplar()),
        "net.probe" => Arc::new(gridwatch_store::keys::net::Probes {
            targets: vec![gridwatch_store::keys::net::ProbeStat {
                target: "gateway".into(),
                addr: "192.168.100.1".into(),
                kind: gridwatch_store::keys::net::ProbeKind::Icmp,
                min_ms: 1.2,
                avg_ms: 1.4,
                max_ms: 2.0,
                mdev_ms: 0.1,
