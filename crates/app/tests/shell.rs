...
fn stale_sources_are_badged_after_three_cadences() {
    ...
    // while silent → 3 s; arc 5a), sensors (1 s → 3 s; arc 5b) and net
    // (1 s → 3 s; arc 7) are.
    sh.set_clock(Ts(64_000_000_000));
    let partly = page_text(&mut sh, 250, 70);
    assert_eq!(partly.matches("stale 4s").count(), 5, "{partly}");
    sh.set_clock(Ts(70_000_000_000));
    let all = page_text(&mut sh, 250, 70);
    assert_eq!(all.matches("stale 10s").count(), 6, "{all}");
...
fn stale_threshold_follows_the_configured_cadence() {
    ...
    // (0.5 s → 1.5 s), audio (1 s while silent → 3 s), sensors (1 s → 3 s)
    // and net (1 s → 3 s) are stale, cpu (1.5 s → 4.5 s) not yet.
    sh.set_clock(Ts(64_000_000_000));
    let text = page_text(&mut sh, 250, 70);
    assert_eq!(text.matches("stale 4s").count(), 4, "{text}");
    ...
    let text = page_text(&mut sh, 250, 70);
    // The audio, sensors and net tiles' badges remain (arcs 5 and 7).
    assert_eq!(text.matches("stale").count(), 3, "{text}");
    assert!(
