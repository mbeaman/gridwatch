...
pub fn keys_doc() -> String {
    ...
    out.push_str("> **Generated by `gridwatch keys` (D33) — do not edit.** CI regenerates this file and fails on drift. Labels: `{n}` is an index (core, device, pin), `{text}` a name (`chip:label`, an interface). The **labels** column (D61) says whether the kernel or the config can add and remove this key's labels while gridwatch runs: retention removes the series of a `dynamic` key whose label has been quiet for `max_age` (10 minutes), and never touches a `static` one.\n\n");
    out.push_str("| key | kind | unit | source | labels | meaning |\n|---|---|---|---|---|---|\n");
    for meta in gridwatch_store::CATALOGUE.iter().flat_map(|d| d.iter()) {
        out.push_str(&format!(
            ...
            meta.source.0,
            match meta.labels {
                gridwatch_store::LabelSet::Static => "static",
                gridwatch_store::LabelSet::Dynamic => "dynamic",
            },
            meta.doc.replace('|', "\\|")
        ));
    }
...
/// `gridwatch keybindings` (D59 seam 1): the global key table and every
/// component's declared keys as `docs/KEYBINDINGS.md`. CI regenerates this
/// file and fails on drift, exactly as it does for the metric catalogue — so
/// a component that adds a key documents it by declaring it.
pub fn keybindings_doc(registry: &Registry) -> String {
    use gridwatch_ui::keys::{GLOBAL, Mode};
    let mut out = String::new();
    out.push_str("# Keys\n\n");
    out.push_str(
        "> **Generated by `gridwatch keybindings` (D33/D59) — do not edit.** CI regenerates \
         this file and fails on drift. The global table is `gridwatch-ui::keys::GLOBAL`, which \
         also draws the key bar and the `?` overlay; a component's keys are its \
         `Manifest.keys`, so a component that adds a key documents it by declaring it.\n\n",
    );
    for mode in [Mode::Always, Mode::Grid, Mode::Edit, Mode::Showcase] {
        let rows: Vec<_> = GLOBAL.iter().filter(|b| b.mode == mode).collect();
        if rows.is_empty() {
            continue;
        }
        out.push_str(&format!("## {}\n\n", mode.title()));
        out.push_str("| key | does |\n|---|---|\n");
        for b in rows {
            out.push_str(&format!(
                "| `{}` | {} |\n",
                b.keys,
    ...
    out.push_str(
        "## Component keys\n\nA tile's keys are live only while it holds the keys — focus it \
         and press `Enter`; `Esc` gives them back. A component with no row here takes no keys \
         of its own.\n\n",
    );
    for def in registry.components() {
        let m = def.manifest;
        if m.keys.is_empty() {
            continue;
        }
    ...
    let quiet: Vec<&str> = registry
        .components()
        .filter(|d| d.manifest.keys.is_empty())
