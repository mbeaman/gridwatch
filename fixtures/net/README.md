...
| `sys/class/net/<if>/…` | `eno1` up at 1000 Mb/s, `wlp7s0` down with an **unreadable** `speed` and a `wireless` directory (which the kernel leaves empty on a down radio — the `README` inside it is only there because git does not track an empty directory), a `br-…` bridge, and `lo` |
